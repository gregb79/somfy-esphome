...
void IrisBinarySensor::update_windy(uint32_t address, bool value) {
  if (this->windy_binary_sensor_ && this->address_ == address) {
    this->windy_binary_sensor_->publish_state(value);
  }
}

void IrisBinarySensor::update_sunny(uint32_t address, bool value) {
  if (this->sunny_binary_sensor_ && this->address_ == address) {
    this->sunny_binary_sensor_->publish_state(value);
  }
}

void IrisBinarySensor::dump_config() {
  ESP_LOGCONFIG(TAG, "Iris Binary Sensor: 0x%" PRIx32, this->address_);
